...
// --- 3. CharacterLifepathRequirement ---
[DataDefinition]
public sealed partial class CharacterLifepathRequirement : LoadoutEffect
{
        ...
        [NotNullWhen(false)] out FormattedMessage? reason)
    {
        ...
        var protoManager = collection.Resolve<IPrototypeManager>();
        const string localeString = "character-lifepath-requirement";
        const string color = "green";

        var isValid = Lifepaths.Contains(profile.Lifepath);
        var shouldFail = Inverted ? isValid : (profile.Lifepath == string.Empty || !isValid);
