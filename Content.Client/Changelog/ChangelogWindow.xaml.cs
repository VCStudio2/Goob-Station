...
namespace Content.Client.Changelog
{
    ...
    public sealed partial class ChangelogWindow : FancyWindow
    {
        ...
        private async void PopulateChangelog()
        {
            ...
            //Custom fixed tab
            // var pirateTab = new PirateChangelogTab();
            // Tabs.AddChild(pirateTab);
            // Tabs.SetTabTitle(i++, Loc.GetString($"changelog-tab-title-pirate"));
            var pirateManager = IoCManager.Resolve<PirateChangelogManager>();
            pirateManager.Load();

            if (pirateManager.Changelogs.Count > 0)
            {
                var pirateTab = new PirateChangelogTab();
                pirateTab.PopulateChangelog(pirateManager.Changelogs[0]);
                Tabs.AddChild(pirateTab);
                Tabs.SetTabTitle(i++, Loc.GetString($"changelog-tab-title-pirate"));
            }
            //CrazeTheDragon END
